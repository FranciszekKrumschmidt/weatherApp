...
router.get('/', async (req: Request, res: Response) => {
    ...
    const target_date = req.query.date as string;
    if (!target_date){
    ...
    try {
        ...
        for (const city of cities) {
            ...
            if (row) {
                const last_updated_str = row.last_updated.split(' ')[0];
                if (target_date < today || last_updated_str===today) useCache = true;
            }
            if (useCache) {
                ...
                if (target_date<today){
                    apiUrl = `http://api.weatherapi.com/v1/history.json?key=${API_KEY}&q=${city}&dt=${target_date}`;
                }
                else {
                ...
                const targetDay = apiData.forecast?.forecastday?.find((d: any) => d.date === target_date);
